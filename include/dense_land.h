...
#include "texture.h"
#include "dense_land_id.h"
#include "dense_land_explorer_id.h"
#include "explorer.h"
#include <glm/glm.hpp>
#include <cstdint>
#include <span>
...
/// World data for dense lands (huge dense worlds): cell grid, shared tile atlas, palette. Does not render —
/// pair with a `DenseLandExplorer` for that. Internally a single `IndirectTexture` sized to the
...
/// allocated. See CLAUDE.md "Huge denseLands" for the full design.
class DenseLand
{
public:
    ...
    /// Used by `ExplorerManager<DenseLand>` to clone slot textures via the override-map constructor.
    ...
    /// Used by `DenseLandExplorer` slots when scrolling brings a chunk into view.
    ...
    /// re-sync hot path by `DenseLandExplorer`.
    ...
    /// `DenseLandExplorer` slots compare against their `syncedGeneration` to detect world edits.
...
template<>
struct LandTraits<DenseLand>
{
    using LandId     = DenseLandId;
    using ExplorerId = DenseLandExplorerId;
};
