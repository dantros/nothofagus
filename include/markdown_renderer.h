...
/// Maps a markdown image `src` string (`![alt](src)`) to an engine texture.
/// Return `std::nullopt` to skip the image. See `MarkdownRenderer::setImageResolver`.
using MarkdownImageResolver = std::function<std::optional<TextureId>(std::string_view src)>;
 ...
 *
 * **Inline images (`![alt](src)`)** render an engine texture when an image
 * resolver is installed via `setImageResolver(...)`: the resolver maps the
 * `src` string to a `TextureId`, which is flattened to RGBA and drawn through
 * `ImGui::Image`. Without a resolver (or when it returns `std::nullopt`) the
 * image is skipped. Animated / tile-map sources currently show a single static
 * frame.
 *
...
public:
    ...
    /// Set the resolver that maps an image `src` (`![alt](src)`) to an engine
    /// `TextureId`. The texture is drawn inline via `ImGui::Image`. Pass an
    /// empty `std::function` (or have the resolver return `std::nullopt`) to
    /// skip images. The bound canvas owns the GPU-side bridge.
