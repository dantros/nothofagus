...
static_assert(LandType<DenseLand>);
...
DenseLand::DenseLand(glm::ivec2 mapSize,
                 glm::ivec2 chunkSize,
                 glm::ivec2 tileSize,
...
{
    debugCheck(mapSize.x > 0 && mapSize.y > 0, "DenseLand mapSize must be positive.");
    debugCheck(chunkSize.x > 0 && chunkSize.y > 0, "DenseLand chunkSize must be positive.");
    debugCheck(tileSize.x > 0 && tileSize.y > 0, "DenseLand tileSize must be positive.");
    debugCheck(!tileGraphics.empty(), "DenseLand requires at least one tile graphic layer.");
    ...
    debugCheck(
     ...
     && static_cast<std::int64_t>(chunkSize.y) * static_cast<std::int64_t>(tileSize.y) <= std::numeric_limits<int>::max(),
        "DenseLand chunkSize * tileSize exceeds int range.");
    ...
    for (std::size_t i = 0; i < tileGraphics.size(); ++i)
    {
        debugCheck(tileGraphics[i].size() == pixelsPerLayer,
                   "DenseLand tile graphic layer size must equal tileSize.x * tileSize.y.");
        mCache.setPixels(std::span<const std::uint8_t>(tileGraphics[i]), i);
    }
    mCache.setMap(mapSize);

    // The cache's dirty flags will get set by setCell/setMapBulk over the DenseLand's
...
bool DenseLand::inBounds(glm::ivec2 worldCell) const
{
...
bool DenseLand::chunkInBounds(glm::ivec2 chunkPos) const
{
    return chunkPos.x >= 0 && chunkPos.x < mChunkGridSize.x
        && chunkPos.y >= 0 && chunkPos.y < mChunkGridSize.y;
}

void DenseLand::setCell(glm::ivec2 worldCell, std::uint8_t layerIndex)
{
    debugCheck(inBounds(worldCell),
               "DenseLand::setCell coordinate out of world bounds.");
    debugCheck(static_cast<std::size_t>(layerIndex) < mCache.layers(),
               "DenseLand::setCell layer index out of range of registered tile graphics.");
...
std::uint8_t DenseLand::cell(glm::ivec2 worldCell) const
{
    debugCheck(inBounds(worldCell),
               "DenseLand::cell coordinate out of world bounds.");
    return mCache.cell(worldCell.x, worldCell.y);
}

void DenseLand::chunkDataInto(glm::ivec2 chunkPos, std::span<std::uint8_t> out) const
{
    debugCheck(chunkPos.x >= 0 && chunkPos.x < mChunkGridSize.x
            && chunkPos.y >= 0 && chunkPos.y < mChunkGridSize.y,
               "DenseLand::chunkDataInto chunk coordinate out of grid bounds.");
    ...
    debugCheck(out.size() == cellsPerChunk,
               "DenseLand::chunkDataInto output span size must equal chunkSize.x * chunkSize.y.");
...
std::vector<std::uint8_t> DenseLand::chunkData(glm::ivec2 chunkPos) const
{
    const std::size_t cellsPerChunk =
        static_cast<std::size_t>(mChunkSize.x) * static_cast<std::size_t>(mChunkSize.y);
...
std::uint64_t DenseLand::chunkGeneration(glm::ivec2 chunkPos) const
{
    debugCheck(chunkPos.x >= 0 && chunkPos.x < mChunkGridSize.x
            && chunkPos.y >= 0 && chunkPos.y < mChunkGridSize.y,
               "DenseLand::chunkGeneration chunk coordinate out of grid bounds.");
    const std::size_t chunkIdx =
        static_cast<std::size_t>(chunkPos.y) * static_cast<std::size_t>(mChunkGridSize.x) +
        static_cast<std::size_t>(chunkPos.x);
