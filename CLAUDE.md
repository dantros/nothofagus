...
The inverse of "render ImGui into an RTT": sample an engine sprite *from* an ImGui window (`ImGui::Image`). Engine textures are 2D *arrays* / palette-indexed, which ImGui can't sample, so the Visual is rendered into an internal render target (RGBA, via the normal sprite path — Direct / Indirect / tile-map / animation all resolve) and that RTT's color attachment is exposed to ImGui as a flat-2D handle (`ImTextureID`). This is the `ImguiImageManager` ([source/imgui_image_manager.h](source/imgui_image_manager.h)), a sibling of `ImguiRttManager`.

**Images are registered, not drawn inline.** `registerImguiImage(Visual, Spec)` returns a stable `ImguiImageId` ([include/imgui_image_id.h](include/imgui_image_id.h)); you then draw that id each frame. Registration **decouples allocation from display**: the internal RTT is allocated immediately and its `ImTextureID` is created on the next render tick, then persists for the registration's lifetime — so any draw at least one rendered frame after registration is **warm-up-free**, with no churn on size changes or garbage collection. The id is a first-class ImGui image handle, usable in raw `ImGui::Image` too.

```cpp
// Register up front (e.g. at load). Handle ready by the next render tick.
Nothofagus::ImguiImageId iconId = canvas.registerImguiImage(
    Nothofagus::Visual{potionTexId}, ImguiImageSize::Scaled{glm::vec2(4)});      // 4x, crisp

canvas.run([&](float dt) {
    ImGui::Begin("inventory");
    canvas.imguiImage(iconId);                                   // convenience draw (magFilter + opacity honored)
    canvas.imguiImage(iconId, canvas.imguiImageSize(iconId) * 0.5f); // draw-size override: GPU downscale of the fixed-res handle
    // …or drive ImGui yourself — the id resolves to a plain ImTextureID:
    ImGui::Image(static_cast<ImTextureID>(canvas.imguiImageHandle(iconId)),
                 ImVec2(canvas.imguiImageSize(iconId).x, canvas.imguiImageSize(iconId).y));
    ImGui::End();
});

// Live content (animation / opacity / texture swap) keeps the same id and handle:
Nothofagus::Visual v{animTexId}; v.currentLayer() = frame;
canvas.updateImguiImage(iconId, v);   // re-renders next tick; re-warms only if the physical size changes
canvas.unregisterImguiImage(iconId);  // frees RTT + handle + resource pins
```

```cpp
// include/canvas.h
ImguiImageId  registerImguiImage(const Visual& visual, const ImguiImageSize::Spec& = ImguiImageSize::Natural{});
void          updateImguiImage(ImguiImageId id, const Visual& visual);
void          unregisterImguiImage(ImguiImageId id);
void          imguiImage(ImguiImageId id, std::optional<glm::vec2> drawSize = std::nullopt);
std::uint64_t imguiImageHandle(ImguiImageId id) const;   // ImTextureID; 0 if unknown / not yet ready
glm::vec2     imguiImageSize(ImguiImageId id) const;     // resolved logical display size
bool          isImguiImageReady(ImguiImageId id) const;
```
...
**Behavior / rules:**
- **Size Specification (`ImguiImageSize::Spec` — a `std::variant<Natural, Scaled, Explicit>`), two orthogonal axes** — fixes the **rasterization resolution** at registration. The **size source** is the variant alternative; the **units** are an `ImguiImageUnits` field on each (default `Logical`). The default, `ImguiImageSize::Natural{}`, is the visual's **real on-screen size** — its mesh AABB extent with no scale transform (auto-quad → texture size; custom mesh → its full extent). `ImguiImageSize::Scaled{factor}` (a `glm::vec2` multiplier) multiplies that; `ImguiImageSize::Explicit{size, ImguiImageFit::Fit|Stretch}` renders at an explicit size (`Fit` = uniform-scale + letterbox/pillarbox, `Stretch` = fill/distort). In `Logical` units (DPI-scaled, like the rest of the UI) the off-screen target is **rasterized at the chosen size × `contentScale()`** (the same DPI density the font atlas uses, via `style.FontScaleDpi`), so **mesh geometry is rasterized at the displayed size — crisp edges, not bitmap-upscaled**. In `Device` units the size is in **physical/device pixels** — **1 texel → 1 display pixel** regardless of OS DPI (rasterized at the device size directly, no `contentScale` multiply): e.g. `Natural{ImguiImageUnits::Device}` shows a texture at its native resolution (no size needed), and `Scaled{4, ImguiImageUnits::Device}` is a crisp 4× pixel-art zoom (exactly 4×native physical px). A `Device`-units image appears physically smaller than its `Logical` counterpart on HiDPI. Texture magnification happens through the engine sprite path honoring the texture's own `magFilter`; a non-square custom mesh keeps its proportions under `Fit`.
- **Responsive draw size.** The spec fixes the rasterization resolution; for layouts that resize, vary only the **draw** size via `imguiImage(id, drawSize)` — a cheap GPU sample of the fixed-resolution handle. Register at a generous resolution and downscale at draw time (this is the basis for markdown fit-to-width); upscaling beyond the registered size blurs, so size the registration for the largest you'll show.
...
- **Per-image width bounds** are fractions of the **column width** (the markdown text-wrap width — *not* the width remaining on the current line, so an inline image near a line wrap is sized like any other). `maxWidthPercentage` (default `1.0`) is a **ceiling** — `width = min(intrinsic, maxPct·column)`, always a lossless downscale. `minWidthPercentage` (default `0.0`) is a **floor** — `width = max(intrinsic, minPct·column)`; enlarging past the registered rasterization size upscales the handle (crisp for `Nearest`, soft for `Linear` — register larger if a floored-up image must stay crisp). Height follows aspect. The defaults draw the image at its natural size, capped at the column width.
...
| `hello_imgui_visual.cpp` | Drawing Visuals inside ImGui across every size mode via the registration model (`registerImguiImage` + `imguiImage`): `Natural` / `Scaled` / `Device` units, `Explicit` `Fit` vs `Stretch` on a custom mesh, animated paletted visual via `updateImguiImage`, Nearest-vs-Linear sampling (texture `magFilter`), a scale slider through the `imguiImage(id, drawSize)` override, opacity, and a render target as the source (`renderTo` → registered image, sampled like a main bellota's RTT)|
