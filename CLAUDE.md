...
canvas.run([&](float) {
    ...
    canvas.imguiVisual(portrait, ImguiImageSize::LogicalPixels{{120, 80}, ImguiImageFit::Fit}); // fit a logical box
    canvas.imguiVisual(icon, ImguiImageSize::DevicePixels{{32, 32}}); // exactly 32x32 screen px
    ImGui::End();
});
```
...
**Behavior / rules:**
- **Size Specification (`ImguiImageSize::Spec` — a `std::variant<Natural, Scaled, LogicalPixels, DevicePixels>`).** The first three size in **logical pixels** (scale with OS DPI); `DevicePixels` sizes in **physical/device pixels** (bypasses DPI). `LogicalPixels` and `DevicePixels` are the same shape (explicit `size` + `fit`), differing only in units. The default, `ImguiImageSize::Natural{}`, is the visual's **real on-screen size** — its mesh AABB extent with no scale transform (auto-quad → texture size; custom mesh → its full extent). `ImguiImageSize::Scaled{factor}` (a `glm::vec2` multiplier) multiplies that; `ImguiImageSize::LogicalPixels{size, ImguiImageFit::Fit|Stretch}` renders at an explicit logical size (`Fit` = uniform-scale + letterbox/pillarbox, `Stretch` = fill/distort); `ImguiImageSize::DevicePixels{size, fit}` renders at an explicit size in device pixels — **1 texel → 1 display pixel** regardless of OS DPI, so a 100×100 texture at `{100,100}` is exactly 100×100 screen pixels (and appears physically smaller than a logical-pixel image on HiDPI). For the logical modes the off-screen target is **rasterized at the chosen size × `contentScale()`** (the same DPI density the font atlas uses, via `style.FontScaleDpi`), so **mesh geometry is rasterized at the displayed size — crisp edges, not bitmap-upscaled** — and texture magnification happens through the engine sprite path honoring the texture's own `magFilter`. A non-square custom mesh keeps its proportions under `Fit`. (`DevicePixels` rasterizes at the device size directly, no `contentScale` multiply.)
