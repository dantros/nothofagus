...
canvas.run([&](float) {
    ...
    canvas.imguiVisual(portrait, ImguiImageSize::Explicit{{120, 80}, ImguiImageFit::Fit}); // fit a logical box
    canvas.imguiVisual(icon, ImguiImageSize::Natural{ImguiImageUnits::Device}); // native res, 1 texel -> 1 screen px
    ImGui::End();
});
```
...
void imguiVisual(const Visual& visual, const ImguiImageSize::Spec& sizeSpec = ImguiImageSize::Natural{});
```
